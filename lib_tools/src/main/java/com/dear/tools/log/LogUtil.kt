...
import com.dear.tools.DeviceUtil
...
//        logger.init(application, isDebug, logPath, namePrefix)
        ...
        val envInfo = """
            *******************************
            ${DeviceUtil.phoneManufacturer}_${DeviceUtil.phoneModel}_${metrics.widthPixels}x${metrics.heightPixels}_${metrics.densityDpi}_${Build.VERSION.SDK_INT}
            *******************************
        """.trimIndent()
//        logger.e(TAG, envInfo)
    ...
    fun w(throwable: Throwable? = null, saveLog: Boolean = false) {
        prepareLog(Log.WARN, LogTag.WARN, "", throwable, saveLog)
    }

    @JvmOverloads
    fun e(throwable: Throwable? = null, saveLog: Boolean = false) {
        prepareLog(Log.ERROR, LogTag.ERROR, "", throwable, saveLog)
    }
