...
abstract class RecycleViewAdapter<M, VB : ViewBinding> :
    ...
    override fun getItemViewType(position: Int): Int {
        return if (showEmpty) {
            if (position == 0 && headerView != null) {
                //当前view是头部信息
                ItemViewType.TYPE_HEADER.value
            } else if (position == 0 && headerView == null) {
                //当前数据空位,展示空页面
                ItemViewType.TYPE_EMPTY.value
            ...
            if (position == 0 && headerView != null) {
                ...
                //当前view是底部信息
                ItemViewType.TYPE_FOOTER.value
            } else if (position == (itemCount - 1) && headerView == null && footerView != null) {
                //当前view是底部信息
                ItemViewType.TYPE_FOOTER.value
            } else {
    ...
    fun getCenterViewType(position: Int): Int {
        return ItemViewType.TYPE_NORMAL.value
    }
            ...
            manager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
                override fun getSpanSize(position: Int): Int {
                    return when (ItemViewType.enumOfValue(getItemViewType(position))) {
                        ItemViewType.TYPE_HEADER, ItemViewType.TYPE_EMPTY, ItemViewType.TYPE_FOOTER -> {
                            manager.spanCount
                        }
